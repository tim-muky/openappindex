#!/usr/bin/env python3
"""
Finishing pass for the hop-2 delta — identical mechanics to enrich_new_apps.py,
separate files so each hop's contribution stays attributable:

  closure_hop2_new.json  →  hop2_meta.json · hop2_sleep.json · hop2_prices.json
...
ids=list(new.keys())
print(f"hop-2 keyword candidates to look up: {len(ids)}",flush=True)
...
json.dump(meta,open("data/hop2_meta.json","w",encoding="utf-8"),ensure_ascii=False)
...
print(f"classified on full descriptions: v1 excludes {v1_out} · v2 excludes {v2_out} "
      f"· HOP-2 NEW SLEEP/MEDITATION APPS: {len(kept)}",flush=True)
json.dump(kept,open("data/hop2_sleep.json","w",encoding="utf-8"),ensure_ascii=False,indent=1)
