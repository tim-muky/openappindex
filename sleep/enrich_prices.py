#!/usr/bin/env python3
"""Fetch in-app purchase prices for the sleep/meditation slice.

Same reader as sample/enrich_prices.py: the price list Apple publishes on each
product page. No reviews, no third-party content, no personal data. Where a
price cannot be read the record says so - never guessed. Resumable: skips ids
...
OUT="data/prices_sleep_de.json"

apps=json.load(open("data/sleep_slice.json",encoding="utf-8"))
done=json.load(open(OUT,encoding="utf-8")) if os.path.exists(OUT) else []
have={r["track_id"] for r in done}
todo=[a for a in apps if a["trackId"] not in have]
print(f"slice {len(apps)} apps · already read {len(have)} · to fetch {len(todo)}",flush=True)
